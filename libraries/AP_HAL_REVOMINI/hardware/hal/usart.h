...
static inline void usart_use_tx_fifo(const usart_dev *dev, uint8_t enable) {
	dev->state->usetxrb= (enable > 0);
}

static inline void usart_use_timeout(const usart_dev *dev, uint8_t enable) {
	dev->state->use_timeout=(enable > 0);
}

static inline void usart_set_timeout(const usart_dev *dev, uint32_t timeout) {
	dev->state->tx_timeout = timeout;
}
 ...
 */
static inline void usart_putc(const usart_dev* dev, uint8_t bt) {
	//uint32_t ret=0;
	//uint32_t cnt=0;
