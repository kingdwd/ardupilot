...
class REVOMINI::NRF_parser : public REVOMINI::_parser {
public:
    NRF_parser() {}
    ...
    inline uint8_t  get_valid_channels() { noInterrupts(); uint8_t t=  channels;  interrupts(); return t; }
    inline uint16_t get_val(uint8_t ch)  { noInterrupts(); uint16_t t= rcData[ch]; interrupts(); return t; }
...
private:
    ...
    void cs_assert(){ nrf->set_speed(AP_HAL::Device::SPEED_HIGH); REVOMINIGPIO::_write(BOARD_NRF_CS_PIN, 0); }
    void cs_release(){                                            REVOMINIGPIO::_write(BOARD_NRF_CS_PIN, 1); }
