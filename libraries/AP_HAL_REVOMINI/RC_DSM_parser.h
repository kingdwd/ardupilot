...
public:
    ...
    inline uint64_t get_last_signal() { noInterrupts(); uint64_t t= _dsm_last_signal; interrupts(); return t; } 
    inline uint64_t get_last_change() { noInterrupts(); uint64_t t=  last_dsm_change; interrupts(); return t; }
    inline uint8_t  get_valid_channels() { noInterrupts(); uint8_t t=  _dsm_channels; interrupts(); return t; }
    inline uint16_t get_val(uint8_t ch)  { noInterrupts(); uint16_t t=  _dsm_val[ch]; interrupts(); return t; }
    
    bool bind(int dsmMode) override;
